...
#include "../RustSDK/ProtoBuf/ProtoBuf.h"

#include "../RustHooks/HookFunc/HookFunc.h"

#include <array>
struct fired_projectile {
	AssemblyCSharp::Projectile* original;
	AssemblyCSharp::Projectile* fake;
	float fired_at;
	int updates;
};
inline std::array<fired_projectile, 32> fired_projectiles;
...
namespace Predictionz {
	struct
	{
		std::array<Vector3, 32> vectorArray;
...
inline void SimulateProjectile(BasePlayer* target,
	...
	while (simulations < m_options.m_options.Simulations) {
		travel_t = 0.f;
...
{
	//auto EyesPosition = AssemblyCSharp::LocalPlayer::get_Entity()->eyes()->get_position();

	auto Base_Projectile = Local->GetHeldEntityCast<AssemblyCSharp::BaseProjectile>();
	...
	auto fProjectilePrefab = mod->projectileObject()->Get()->GetComponent<AssemblyCSharp::Projectile>(System::Type::Projectile());
	...
	// 30 Sims
	for (size_t j = 0; j < m_options.m_options.Simulations; j++)
	{
...
inline Vector3 start_pos;

inline Vector3 targetpos = Vector3();
inline Vector3 bullettppos = Vector3();
inline Vector3 cachedTargetPos = Vector3(0, 0, 0);
